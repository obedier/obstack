...
Ten opinionated workflow skills for [Claude Code](https://docs.anthropic.com/en/docs/claude-code). PMF analysis, plan review, UX review, code review, one-command shipping, browser automation, QA testing, and engineering retrospectives — all as slash commands.
...
|-------|------|--------------|
| `/pmf-review` | Product strategist | Pressure-test PMF: market sizing, moat analysis, killer features, GTM plan. |
...
| `/ux-review` | Senior UX lead | Lock in IA, journeys, states, trust, accessibility, and responsive behavior before implementation. |
...
> Install gstack: run `git clone https://github.com/garrytan/gstack.git ~/.claude/skills/gstack && cd ~/.claude/skills/gstack && ./setup` then add a "gstack" section to CLAUDE.md that says to use the /browse skill from gstack for all web browsing, never use mcp\_\_claude-in-chrome\_\_\* tools, and lists the available skills: /pmf-review, /plan-ceo-review, /plan-eng-review, /ux-review, /review, /ship, /browse, /qa, /setup-browser-cookies, /retro. Then ask the user if they also want to add gstack to the current project so teammates get it.

### Step 2: Add to your repo so teammates get it (optional)

> Add gstack to this project: run `cp -Rf ~/.claude/skills/gstack .claude/skills/gstack && rm -rf .claude/skills/gstack/.git && cd .claude/skills/gstack && ./setup` then add a "gstack" section to this project's CLAUDE.md that says to use the /browse skill from gstack for all web browsing, never use mcp\_\_claude-in-chrome\_\_\* tools, lists the available skills: /pmf-review, /plan-ceo-review, /plan-eng-review, /ux-review, /review, /ship, /browse, /qa, /setup-browser-cookies, /retro, and tells Claude that if gstack skills aren't working, run `cd .claude/skills/gstack && ./setup` to build the binary and register skills.
...
- Symlinks at `~/.claude/skills/browse`, `~/.claude/skills/qa`, `~/.claude/skills/ux`, `~/.claude/skills/review`, etc. pointing into the gstack directory
...
This is my **product strategist mode**.

Before I write a plan, before I think about architecture, before I open an editor — I need to know if this thing should exist at all. That is what `/pmf-review` is for.

It runs a full product-market fit analysis using the frameworks that built Google, Facebook, Amazon, and thousands of YC companies. Not vibes — quantified analysis:

* **Amazon Working Backwards:** Write the press release first. If it doesn't excite you, stop.
* **Bottom-up market sizing:** Count actual customers, not "the AI market is $500B."
* **Competitive moat scoring:** Network effects, data advantages, switching costs — scored and ranked.
* **Killer feature test:** Is this 10x better on any axis? Can you demo it in 60 seconds?
* **Go-to-market plan:** Channel-product fit, first 100 users strategy, pricing.
* **Unit economics:** LTV:CAC, payback period, default alive or default dead.

Three modes: **NEW PRODUCT** (full analysis), **FEATURE BET** (impact within existing product), **PIVOT** (what you're leaving behind vs. gaining).

It produces a normalized PMF Scorecard where every dimension is rated /5 with clear thresholds: Strong (4+), Promising (3-4), Weak (2-3), Not Yet (<2). No hand-waving.

### Example

Say I have an idea for a playtesting platform for indie game developers.

```
You:   /pmf-review indie game playtesting marketplace

Claude: [Writes press release, evaluates timing, assesses team fit]
        Mode? NEW PRODUCT / FEATURE BET / PIVOT

You:    NEW PRODUCT

Claude: Problem Severity: 16/25 (mapped: 3.2/5) — moderate.
        Current alternatives exist but are bad (Reddit, Discord, $5K labs).
        TAM: $150M (250K indie devs × $600/yr). SOM Y3: $6.4M.
        Moat: 9/30 — CRITICAL. No defensible advantage yet.
        Killer feature: heatmaps + drop-off analytics scored 4.0/5.

        Preliminary PMF Signal: 3.1/5 — Promising, but moat is the
        critical weakness. Recommend building data moat first.

        Continue to GTM and unit economics?
```

That is `/pmf-review`. Not "is this a good idea?" but **"does the math work, and what specifically needs to be true for this to succeed?"**
...
This is my **senior UX lead mode**.

Once the product direction and technical plan are set, I want a UX-specific brain before implementation starts. Not a visual design pass — a structural UX review that catches the problems that cause real user confusion, lost trust, and support tickets.

This mode locks in:

* information architecture and navigation
* core journey clarity and hierarchy
* state coverage (empty, loading, error, success, warning, disabled, permission-blocked)
* trust and transparency for sensitive actions
* accessibility and responsive behavior
* pattern consistency across the product

It works interactively — section by section, one issue per question, with concrete lettered options and opinionated recommendations. It starts with a scope challenge (do we even need all this UX surface area?) and ends with a failure mode audit and TODO review.

### Example

Say I am building a new bulk refund workflow for an admin payments dashboard.

`/ux-review` should ask before anything else:

* Does the existing single-refund flow already solve most of this?
* Does the existing table already support row selection?
* Is a new wizard screen really needed, or is this a table-adjacent action?

Then it walks through architecture/IA, interaction quality, state coverage, and accessibility — pausing after each section for decisions. Every issue gets a severity, a concrete fix recommendation, and tradeoff analysis.

That is `/ux-review`.

Not "make it prettier."
**Make the experience clearer, safer, and buildable.**
...
Re-copy from global: `for s in browse pmf-review plan-ceo-review plan-eng-review ux review ship retro qa setup-browser-cookies; do rm -f .claude/skills/$s; done && rm -rf .claude/skills/gstack && cp -Rf ~/.claude/skills/gstack .claude/skills/gstack && rm -rf .claude/skills/gstack/.git && cd .claude/skills/gstack && ./setup`
...
> Uninstall gstack: remove the skill symlinks by running `for s in browse pmf-review plan-ceo-review plan-eng-review ux review ship retro qa setup-browser-cookies; do rm -f ~/.claude/skills/$s; done` then run `rm -rf ~/.claude/skills/gstack` and remove the gstack section from CLAUDE.md. If this project also has gstack at .claude/skills/gstack, remove it by running `for s in browse pmf-review plan-ceo-review plan-eng-review ux review ship retro qa setup-browser-cookies; do rm -f .claude/skills/$s; done && rm -rf .claude/skills/gstack` and remove the gstack section from the project CLAUDE.md too.
